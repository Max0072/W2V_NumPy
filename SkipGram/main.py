...
def loss(probs, target_ids):
    total_loss = 0
    for target_id in target_ids:
        total_loss += -np.log(probs[target_id])
...
class SkipGram:
    ...
    def build_vocab(self, words):
        unique_words, counts = np.unique(np.array(words), return_counts=True)
    ...
    def forward(self, token_id):
        emb = self.W_in[token_id]
        logits = emb @ self.W_out
        probs = softmax(logits)
        info = (token_id, emb, probs)
        return probs, info

    def backward(self, info, target_ids):
        token_id, emb, probs = info
        y = multi_hot(target_ids, self.vocab_size) / len(target_ids)    #(V,)

        dlogits = (probs - y).astype(np.float32)    #(V,)

        dW_out = np.outer(emb, dlogits)           # (E, 1) @ (1, V) = (E, V)
    ...
    def fit(self, corpus, epochs=50, window=2, lr=0.05):
        ...
        for epoch in range(epochs):
            ...
            for i, (word_id, target_ids) in enumerate(pairs):
                probs, info = self.forward(word_id)
                total_loss += loss(probs, target_ids)
                dW_in, dW_out = self.backward(info, target_ids)
